...
function FarmerProfile() {
  ...
  const profile_url = `http://127.0.0.1:8000/agriLink/single_profile/${encodedUserId}`;
  
  const [showModal, setShowModal] = useState(false);
  const [dateFormat, setDateFormat] = useState();
  const [formData, setFormData] = useState({user:user.user_id, image:null, contact:0, location:'', bio:''});
